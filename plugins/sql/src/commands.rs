...
pub(crate) async fn load<R: Runtime>(
    ...
    db_instances.0.write().await.insert(db.clone(), crate::DbInstance::new(db.clone(), pool));
...
pub(crate) async fn execute(
    ...
    // Unlocking db_instances before performing the query allows new databases to be loaded and new
    // connections to be acquired during the active query.
    let conn = {
        let instances = db_instances.0.read().await;
        let db_instance = instances.get(&db).ok_or(Error::DatabaseNotLoaded(db.clone()))?;
        db_instance.get_connection(connection_id)?
    };
